...
- Nightly canary: **SCHEDULE PAUSED (2026-09-17) for the A.2 gate** — its 00:30 UTC run drained the fresh Groq exec window and the A.2 battery starved (13/22 covered questions 429-aborted, 09-17 night). Manual `workflow_dispatch` runs stay available; restore the cron after the gate clears or when non-Groq exec capacity lands. First dispatched run: 34749896271.
- Keep-alive (FIXED 2026-09-17): was pinging `/health` every 10 min — `/health` touches the DB, so ~4,300 pings/month reset Neon's 5-min idle timer, defeated scale-to-zero, and burned the ENTIRE free compute-hour allowance (~180 CU-hrs of the ~190/mo) — the root cause (with heavy live testing) of the mid-month Neon quota wall that took the database and the A.2 battery down. The workflow now pings `/live` (dependency-free liveness); the nightly shadow battery is the deep DB health check. Render cold-start elimination unchanged.
- TokenRouter: **LANE RETIRED (2026-09-17, owner decision)** — `z-ai/glm-5.3-free` vanished from their catalog (upstream rotation; the endpoint lives but the free model is gone). Free catalogs are not contracts — the ADR-008 lesson again. The lane is removed from .env, .env.example, canary.yml, and shadow_battery.yml (which now assembles lanes from present secrets instead of shipping static JSON).
- ModelScope second backup: **PARKED — external constraint** (2026-09-10). API-Inference requires Alibaba Cloud account binding + real-name verification (KYC); verified live (token lists 46 models, inference 401s until KYC completes). Revisit only if the active lanes prove insufficient.
- Exec-vetting 2026-09-18 (paced gauntlet, scripts/vet_exec_candidate.py + vet_exec_report.json): **z-ai/glm-5.2:free REJECTED for the exec peer pool on evidence** — auditor quality PROVEN (tamper battery 4/4 corruptions rejected, baseline certified with gold figure, zero fabrications anywhere) but the ADR-008 bar ("passes the benchmark on the live pipeline") NOT met: corpus-scale runs refused at 443-464s under upstream shared-pool congestion (limit_source=upstream_provider_shared_pool, provider Decart). Demoted to LAST fleet failover lane. unbiased/pareto (stealth/union-alpha's successor after its 2026-09-18 retirement) is the real exec expansion — paid $2.5/$7.5 per M, needs account credits to vet.
- stealth/union-alpha: **RETIRED UPSTREAM 2026-09-18** mid-vetting ("This model was Unbiased's Pareto") — third live-caught provider rotation. IDs replaced everywhere (chain, workflows, advisor) within minutes; the fail-closed fabric refused correctly on the dead ID (zero tokens wasted).
- freellmapi.co: **RESERVE LANE candidate (2026-09-17)** — an OpenRouter-style router (370 free models / 24 providers per its catalog page) but the API needs a signup key; unprobed. Chain is complete without it.
